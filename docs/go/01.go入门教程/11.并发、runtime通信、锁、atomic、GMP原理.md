---
title: 11-并发、runtime通信、锁、atomic、GMP原理
date: 2023-01-12 15:42:42
permalink: /pages/8fa5c0/
categories:
  - go
  - go入门教程
tags:
...
# 总结
- runtime: 用户态的使用。 对比java减少内核态的切换
- channel: 通信共享内存数据， 而不是加锁的方式（使得共享内存来实现通信）。
> 同步通道[不带容量]，有缓存通道[有容量,只有满了才阻塞]，接收和发送都会阻塞。
> 接受用for range 自动判断通道关闭。
> 单向通道：[限定作用,用途清楚,编码最好指定]在函数传参及任何赋值操作中将双向通道转换为单向通道是可以的，但反过来是不可以的。
> select: 同时从多个通道接收数据。带有随机性。可以用于判断管道是否存满。
- 锁: 和java类似互斥锁、读写锁
- atomic： 同java。
- GMP原理：go语音的核心。
