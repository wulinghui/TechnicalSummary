---
title: 07-函数、闭包、延迟调用、异常处理
date: 2023-01-12 15:42:37
permalink: /pages/30cdf6/
categories:
  - go
  - go入门教程
tags:
...
func main() {
	...
	for i := range whatever {
		defer fmt.Println(i) //打印 5/4/3/2/1  先进后出的
	}
}
~~~
...
> 可变参数同java。
- 匿名函数: 类似js,做回调函数。
- 闭包: 同js,用法重点不一样,这里为了共享局部变量的值.【js是为了局部变量不被外部感知】
- defer,延迟调用:   先进后出执行【天然的栈】。立刻拷贝函数中引用的外部参数（用匿名函数解决）。  
- 异常处理:  pani、recover 函数。errors.New、fmt.Errorf创建error对象。Try 函数的回调。 